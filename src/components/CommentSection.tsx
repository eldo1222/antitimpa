...
export const CommentSection: React.FC<CommentSectionProps> = ({
  ...
  const activeUser = currentUser ? {
    ...
    email: currentUser.email || `${currentUser.username}@antitimpa.id`,
    photoURL: currentUser.avatar || currentUser.photoURL || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=120&auto=format&fit=crop&q=80'
  } : googleUser ? {
    uid: googleUser.uid,
    displayName: googleUser.displayName,
    email: googleUser.email,
    photoURL: googleUser.photoURL || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=120&auto=format&fit=crop&q=80'
  } : null;
      ...
      <div className="flex items-center justify-between border-b border-[#202030] pb-3.5">
        ...
        {activeUser ? (
          <div className="flex items-center gap-2 bg-[#191924] border border-[#2b2b3d] px-2.5 py-1 rounded-xl text-xs">
            ...
            />
            <span className="font-medium text-slate-200 hidden sm:inline">{activeUser.displayName}</span>
            <button
              onClick={() => {
                if (googleUser) logoutGoogle();
                else logout();
              }}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 bg-white hover:bg-slate-100 text-slate-900 font-bold px-3 py-1.5 rounded-xl text-xs shadow-sm transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="text-xs text-[#ff5b14] hover:underline font-bold px-2 py-1 cursor-pointer"
            >
              Login Manual
            </button>
          </div>
